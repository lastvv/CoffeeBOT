...
async def answer_start_command(message: types.Message, User):
    print(User)
    await message.answer(text=f'{hunderline("Здарова")}, {hbold(message.from_user.first_name)}👋'
                              f'\n{hitalic("Наконец-то ты здесь😉")}',
                         reply_markup=start_inline_keyboard)


@dp.message_handler(text=['Найти марку'])
async def get_item_name(message: types.Message):
    await message.answer(text='Какую марку ты хочешь найти?')
...
@dp.message_handler(text=['Найти марку'])
async def get_item_name(message: types.Message):
    await message.answer(text='вот тебе список:'
                              'f\n 1.Нескафе')
    await ByerState.wait_item_name.set()
...
async def get_item_command(message: types.Message, state: FSMContext):
    ...
    if not item_info:
        ...
        await message.answer(text=f'Название марки кофе {item_info["name"]}:' \
                                  f'\nсредняя цена: {item_info["count"]}' \
                                  f'\nОписание: {item_info["description"]}')
...
async def answer_item_command(message: types.Message):
    await message.answer(text=f'{hunderline("Ԟапучи́но")}☕ '
                              f'\nкофейный напиток итальянской кухни'
                              f' на основе эспрессо с добавлением в него '
                              f' подогретого вспененного молока.',
                         reply_markup=info_default_keyboard)
...
async def answer_item_command(message: types.Message):
    await message.answer(text=f'{hunderline("Раф кофе ")}☕ ''– это взбитая в капучинаторе смесь из '
...
async def answer_item_command(message: types.Message):
    await message.answer(text=f'{hunderline("Ла́тте ")}☕— кофейный напиток родом из Италии,'
...
async def answer_item_command(message: types.Message):
    ...
    if message.from_user.id == message.contact.user_id:
        await message.answer(text='Ну всё, мы тебя записали, жди')
    else:
